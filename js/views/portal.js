import { CONFIG, SESSION_STATUSES } from '../config.js';
...
async function showView(view) {
  ...
  const renderers = { overview: overviewView, accounts: accountsView, submissions: submissionsView, 'player-records': playerRecordsView, 'team-records': teamRecordsView, applications: applicationsView, workflows: workflowsView, sessions: sessionsView, gangs: gangsView, content: contentView, projects: projectsView, rules: rulesView, settings: settingsView, backup: backupView, audit: auditView };
  try { content.innerHTML = await (renderers[view] || overviewView)(); bindViewActions(view); }
  catch (error) { content.innerHTML = `<div class="permission-lock"><h2>Bereich konnte nicht geladen werden</h2><p>${escapeHtml(error.message)}</p></div>`; }
}
...
function bindViewActions(view) {
  ...
  document.querySelectorAll('[data-announce-session]').forEach(button=>button.addEventListener('click',async()=>{try{const response=await fetch('/api/discord-webhook',{method:'POST',headers:{'content-type':'application/json',authorization:authState.session?.access_token?`Bearer ${authState.session.access_token}`:''},body:JSON.stringify({sessionId:button.dataset.announceSession})});const result=await response.json();if(!response.ok)throw new Error(result.error||'Webhook fehlgeschlagen');toast('Discord-Ankündigung gesendet','','success');}catch(error){toast('Ankündigung nicht gesendet',error.message,'error');}}));
